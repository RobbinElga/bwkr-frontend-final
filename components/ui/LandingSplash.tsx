...
export function LandingSplash() {
    ...
    useEffect(() => {
        const hold = setTimeout(() => setDone(true), 1200); // tampil ~1.2 dtk
        return () => clearTimeout(hold);
    }, []);

    useEffect(() => {
        if (!done) return;
        const t = setTimeout(() => setGone(true), 550); // tunggu fade selesai
    ...
    return (
        <div className={`fixed inset-0 z-[300] flex items-center justify-center overflow-hidden bg-deep-forest transition-opacity duration-500 ${done ? "pointer-events-none opacity-0" : "opacity-100"}`}>
